...
const MenPowerListings = () => {
  ...
  // Auto-scroll effect (LEFT to RIGHT) with 3s pause
  // Auto-scroll effect (LEFT to RIGHT) with 3s pause
  useEffect(() => {
    const autoScrollInterval = setInterval(() => {
      ...
      // Check if we've reached the start (scrolled all the way left)
      const isAtStart = container.scrollLeft <= 10
      
      if (isAtStart) {
        // Reset to end instantly
        container.scrollTo({ 
  ...
  // Initialize scroll position to the end for LEFT to RIGHT effect
  useEffect(() => {
    const container = scrollContainerRef.current
    if (container) {
  ...
  return (
    <section 
      className="py-16 px-5 bg-slate-50"
    >
      {/* Section Header */}
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-4xl font-black text-[#0A1128]">Men Power</h2>
        <Link to="/men-power/list" className="px-6 py-2.5 bg-white border border-slate-200 text-[#0A1128] rounded-lg font-bold text-sm hover:bg-slate-50 transition-all shadow-sm">
          Show All →
        </Link>
      </div>
            ...
            >
              <div className="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
                ...
                <div className="p-5 space-y-3">
                  ...
                  {/* View Details Button */}
                  <button className="w-full mt-3 py-2.5 bg-slate-50 text-[#0A1128] rounded-lg font-bold text-sm hover:bg-orange-600 hover:text-white transition-all">
                    View Details
                  </button>
                </div>
              </div>
...
export default MenPowerListings
